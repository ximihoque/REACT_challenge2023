...
def train(args, model, train_loader, optimizer, criterion):
    ...
    for batch_idx, (_, _, _, speaker_emotion, _, _, _, listener_emotion, listener_emotion_neg, listener_3dmm, _) in enumerate(tqdm(train_loader)):
        ...
        listener_3dmm_out, listener_emotion_out, \
                distribution, \
                spk_encoded, listener_emt_pos, listener_emt_neg = model(speaker_emotion, (listener_emotion, listener_emotion_neg))
        ...
        with torch.no_grad():
            listener_3dmm_out_, listener_emotion_out_, _ = model(speaker_emotion)
...
def main(args):
    ...
    val_loss, rec_loss, kld_loss = val(args, model, val_loader, val_criterion, render, 0)
